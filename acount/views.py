...
from .forms import ProfileForm, SignupForm, ResendEmailConfirmForm
...
# ویوو ویرایش مقالات
class ArticleUpdateView(LoginRequiredMixin, AccessAuthors, AccessUpdateForm, FormValidMixin, CreateFieldsMixin,
                        UpdateView):
    model = Article
    template_name = 'registration/create_article.html'
...
class Signup(CreateView):
    ...
    # تابعی که قبل از چک شدن ولید بودن فرم فراخوانی میشود ما با اوورراید کردن این تابع
    ...
    #      با ساختن صفحه ی جداگانه برای ارسال مجدد ایمیل این قسمت کامنت شد

# ارسال دوباره ی ایمیل فعالسازی
class ResendEmailConfirm(CreateView):
    form_class = ResendEmailConfirmForm
    template_name = 'registration/resend_email_confirm.html'
...
class UsersProfile(LoginRequiredMixin, AccessAdmins, UpdateView):
    ...
    # و یک تمپلیت به آن میدهیم که فرم ما به آ« وصل شئد
    # به عبارت دیگر ابتدا یوزر را به فرم میفرستیم و س÷س فرم را به تمپلیت میفرستیم
    # اینجا فقط مشخص میکنیم که مدل ما از نوع یوزر است و پایین تر مشخص میشود که کدام یوزر باید به فرم ارسال شود
